...
describe("[4.0] YieldSyncV1EMPStrategy.sol - Setup", async () => {
	...
	describe("function managerUpdate()", async () => {
		...
		it("Should allow manager to be changed..", async () => {
			...
			await expect(strategy.managerUpdate(ADDR_1.address)).to.be.not.reverted;
	...
	describe("function utilizedERC20Update()", async () => {
		it("[auth] Should revert when unauthorized msg.sender calls..", async () => {
			...
			await expect(
				strategy.connect(ADDR_1).utilizedERC20Update(
					[mockERC20A.address],
		...
		it("Should revert if no ETH Value feed set for the utilized ERC20..");
		...
		it("Should not revert when VALID allocation passed..", async () => {
			...
			for (let i: number = 0; i < VALID_INPUTS.length; i++)
			{
				...
				await expect(_YSS.utilizedERC20Update(INPUT.utilizedERC20, INPUT.utilization)).to.be.not.reverted;
	...
	describe("function utilizedERC20DepositOpenToggle()", async () => {
		...
		it("[modifier] Should revert if Strategy is not set..", async () => {
			await expect(strategy.utilizedERC20DepositOpenToggle()).to.be.rejectedWith(ERROR.STRATEGY_NOT_SET);
		});
		...
		it("Should toggle utilizedERC20DepositOpen..", async () => {
			...
			await expect(strategy.utilizedERC20DepositOpenToggle()).to.be.not.reverted;
	...
	describe("function utilizedERC20WithdrawOpenToggle()", async () => {
		...
		it("[modifier] Should revert if strategy is not set..", async () => {
			await expect(strategy.utilizedERC20WithdrawOpenToggle()).to.be.rejectedWith(ERROR.STRATEGY_NOT_SET);
		});
		...
		it("Should toggle utilizedERC20WithdrawOpen..", async () => {
			...
			await expect(strategy.utilizedERC20WithdrawOpenToggle()).to.be.not.reverted;
