			...
			describe("[indirect-call] function utilizedERC20Update() - Utilized ERC20 tokens changed..", async () => {
				...
				it("Should update EMP's utilizedERC20 array to be a union of the strategy's utilizedERC20s..", async () => {
					...
					*/
					...
					await eMPs[0].eMPTransferUtil.updateUtilizedERC20();

					const MockCDepositAmount = ethers.utils.parseUnits(".5", 6);
					const MockDepositAmount = ethers.utils.parseUnits(".5", 18);

					await expect(
						eMPs[0].contract.utilizedERC20Deposit([MockCDepositAmount, MockDepositAmount])
					).to.be.not.reverted;
